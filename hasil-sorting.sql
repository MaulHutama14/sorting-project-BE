...
SELECT tpk.sort_id,tpk.assign_date as START,tpk.assign_end AS END, TIMEDIFF(tpk.assign_end,tpk.assign_date) , tk.nama_komponen, tk.master_nama_produk, ta.nama_alat, tp.nama_proses, tp.sort_id AS urutan_proses 
FROM tx_proses_komponen tpk
JOIN tx_komponen tk ON tk.id=tpk.id_komponen
JOIN tm_proses tp ON tp.id=tpk.id_proses
JOIN tm_alat ta ON ta.nama_alat=tpk.nama_id_alat
ORDER BY  tpk.sort_id ASC, tp.sort_id ASC; -- SEMUA tpk.sort_id ASC, tp.sort_id ASC